...
const CERTS = [
  { domain: "*.myapp.ir",       issuer: "Let's Encrypt",   status: "valid",   expires: "۱۴۰۵/۰۵/۲۰", daysLeft: 84, autoRenew: true  },
  { domain: "*.staging.ir",     issuer: "Let's Encrypt",   status: "valid",   expires: "۱۴۰۵/۰۴/۱۱", daysLeft: 45, autoRenew: true  },
  { domain: "static.partner.ir",issuer: "ZeroSSL",         status: "expiring",expires: "۱۴۰۵/۰۲/۲۵", daysLeft: 18, autoRenew: false },
  { domain: "*.nab.ir",         issuer: "DigiCert",        status: "valid",   expires: "۱۴۰۶/۰۱/۰۵", daysLeft: 340,autoRenew: false },
  { domain: "cdn.myapp.ir",     issuer: "Let's Encrypt",   status: "valid",   expires: "۱۴۰۵/۰۴/۳۰", daysLeft: 64, autoRenew: true  },
];
...
export default function CDNCertificatesPage() {
  return (
    <div className="flex flex-col gap-20 p-20 min-h-full" dir="rtl">
      <div className="glass rounded-16 p-20">
        <div className="flex items-center justify-between mb-14">
          <div>
            ...
            <p className="text-[12px] text-text-muted mt-2">مدیریت گواهی‌های HTTPS برای CDN</p>
          </div>
      ...
      <div className="flex flex-col gap-8">
        {CERTS.map((c) => (
          <div key={c.domain} className={`glass rounded-14 p-16 border ${c.status === "expiring" ? "border-amber-200" : "border-border"}`}>
            <div className="flex items-center gap-12">
              <div className="flex-1">
                <div className="flex items-center gap-10 mb-6 flex-wrap">
                  <p className="text-[13px] font-bold font-mono text-text-main ltr-text" style={{ direction: "ltr" }}>{c.domain}</p>
                  ...
                  {c.autoRenew && <span className="px-6 py-2 rounded-4 bg-purple-100 text-purple-700 text-[10px]">auto-renew</span>}
                </div>
